...
document.addEventListener("submit", async function(e){
  const form=e.target;
  if(!form) return;
  const probe = form.querySelector("#patientName, [name='patient_name'], [name='patientName']");
  if(!probe) return;
  ...
  function __build() {
  const payload = {};
  payload['patient_name'] = (form.querySelector("#patientName, [name='patient_name'], [name='patientName']")?.value || '').trim();
  payload['category'] = (form.querySelector("#category, [name='category']")?.value || '').trim();
  payload['message'] = (form.querySelector("#message, [name='message']")?.value || '').trim();
  payload['status'] = (form.querySelector("#status, [name='status']")?.value || '').trim();
  return payload;

  }

  try {
    await __apiPost((window.__BASE_URL__ || "") + "/api/feedback", __build());
  } catch(err) {}
}, true);
// ---------------- __AJAX_DB_SYNC ----------------
