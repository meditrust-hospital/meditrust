...
document.addEventListener("submit", async function(e){
  const form=e.target;
  if(!form) return;
  const probe = form.querySelector("#title, [name='title']");
  if(!probe) return;
  ...
  function __build() {
  const payload = {};
  payload['title'] = (form.querySelector("#title, [name='title']")?.value || '').trim();
  payload['message'] = (form.querySelector("#message, [name='message']")?.value || '').trim();
  payload['target_role'] = (form.querySelector("#targetRole, [name='target_role']")?.value || '').trim();
  return payload;

  }

  try {
    await __apiPost((window.__BASE_URL__ || "") + "/api/notifications", __build());
  } catch(err) {}
}, true);
// ---------------- __AJAX_DB_SYNC ----------------
