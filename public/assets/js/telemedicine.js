...
document.addEventListener("submit", async function(e){
  ...
  payload['consult_type'] = (form.querySelector("#consultType, [name='consult_type']")?.value || '').trim();
  payload['datetime'] = (form.querySelector("#datetime, #dateTime, [name='datetime']")?.value || '').trim();
  return payload;

  }

  try {
    await __apiPost((window.__BASE_URL__ || "") + "/api/telemedicine", __build());
  } catch(err) {}
}, true);
// ---------------- __AJAX_DB_SYNC ----------------
