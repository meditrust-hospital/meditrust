...
async function __apiPost(url, payload){
  ...
  if(!res.ok || !data.ok){
    __showToast(data.error || "Request failed");
    throw new Error(data.error || "Request failed");
  }
  __showToast("Saved successfully","success");
  return data;
}
...
document.addEventListener("submit", async function(e){
  const form=e.target;
  if(!form) return;
  const probe = form.querySelector("#patientName, [name='patient_name']");
  if(!probe) return;
  ...
  function __build() {
  const payload = {};
  payload['patient_name'] = (form.querySelector("#patientName, [name='patient_name']")?.value || '').trim();
  payload['doctor_name'] = (form.querySelector("#doctorName, [name='doctor_name']")?.value || '').trim();
  payload['department'] = (form.querySelector("#department, [name='department']")?.value || '').trim();
  payload['visit_date'] = (form.querySelector("#visitDate, [name='visit_date']")?.value || '').trim();
  payload['visit_type'] = (form.querySelector("#visitType, [name='visit_type']")?.value || '').trim();
  payload['revenue'] = (form.querySelector("#revenue, [name='revenue']")?.value || '').trim();
  return payload;

  }

  try {
    await __apiPost((window.__BASE_URL__ || "") + "/api/visits", __build());
  } catch(err) {}
}, true);
// ---------------- __AJAX_DB_SYNC ----------------
