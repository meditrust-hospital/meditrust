...
document.addEventListener("submit", async function(e){
  const form=e.target;
  if(!form) return;
  const probe = form.querySelector("#doctorName, [name='doctor_name']");
  if(!probe) return;
  ...
  payload['diagnosis'] = (form.querySelector("#diagnosis, [name='diagnosis']")?.value || '').trim();
  payload['prescription'] = (form.querySelector("#prescription, [name='prescription']")?.value || '').trim();
  payload['notes'] = (form.querySelector("#notes, [name='notes']")?.value || '').trim();
  return payload;

  }

  try {
    await __apiPost((window.__BASE_URL__ || "") + "/api/emr", __build());
  } catch(err) {}
}, true);
// ---------------- AJAX DB ----------------
